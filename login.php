...
if ($_POST) {
    ...
    # Escapar los datos enviados por el formulario
    $email = $_POST['email'];
    $pass = $_POST['pass'];

    # Consulta SQL para buscar un usuario específico con sentencia preparada
    $sql = "SELECT * FROM usuarios WHERE mail = :email AND pass = :pass AND es_admin = 'S'";

    # Utilizar el método consultar con sentencia preparada
    $params = [
        ':email' => $email,
        ':pass' => $pass
    ];
    $usuarios = $conexion->consultar2($sql, $params);

    # Verificar si se encontró un usuario que coincida con el correo y contraseña
    if (count($usuarios) == 1) {
        # Usuario y contraseña correctos, iniciar sesión
        $_SESSION['usuario'] = "Admin";
        $_SESSION['logueado'] = 'S';
        # Redireccionar porque ingreso ok
